          ...
          {getOrdersAndOrdersItemsApiData?.data?.data?.sort((a: any, b: any) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime()).map((order: any) => (
            <div key={order?.id} className="border border-border rounded-lg overflow-hidden">
              ...
              <div className="p-4">
                <div className="space-y-4">
                  {order?.order_items?.map((product: any) => (
                    <div key={product.id} className="flex items-center gap-4">
                      ...
                      <div>
                        <button
                          className='rounded bg-red-600 text-white font-bold px-4 py-2 hover:bg-red-600 transition-colors'
                ...
                <div className="mt-4 flex py-1 justify-between font-bold hover:text-red-600">
                  <button
