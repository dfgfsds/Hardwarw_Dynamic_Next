...
export default function CartSummary({ totalAmount }: any) {
    ...
    try {
      if (paymentValue === 'cod') {
        ...
        if (paymentAPi) {
          ...
          const options = {
            key: "rzp_live_RBXniX3HVNEgS6",
            amount: final_price * 100,
            currency: "INR",
            name: "Hardware dynamic",
  ...
  const fetchCartAndDeliveryCharge = async () => {
    try {
      ...
      setAddressError("Please Add delivery addess to proceed with checkout.");
    }
  };
  ...
  return (
    <>
      {data?.data?.length ? (
        <div className="space-y-3 mb-3">
          <div className="flex items-center justify-between">
            ...
            <Link
              ...
              className="text-sm text-red-600 font-bold  hover:text-red-600"
        ...
        ${selectedAddressId === address.id ? 'border-red-500 bg-blue-50' : 'border-gray-200 hover:bg-gray-50'}`}
                >
                  <input
                    type="radio"
                    ...
                    className="mt-1 h-4 w-4 text-blue-600 border-gray-300 focus:ring-red-500"
                  />
                  <div className="ml-3">
        ...
        <div className="text-center p-4 bg-gray-50 rounded-lg mb-3">
          ...
          <p className="mt-2 inline-block text-sm text-red-600 hover:text-red-600 font-bold cursor-pointer"
            onClick={() => {
      ...
      <div className="bg-[#F8F7F2] rounded-xl p-3">
        ...
        <div className="mt-6 space-y-4">
          ...
          ${paymentValue === item?.value ? 'border-red-500 bg-blue-50' : 'border-gray-200 hover:bg-gray-50'}`}
              >
                <input
                  type="radio"
                  name="paymentMethod"
                  ...
                  className="mt-1 h-4 w-4 text-red-600 border-gray-300 focus:ring-blue-700"
                />
                <div className="ml-3">
          ...
          {!data?.data?.length ? (
            <button className="w-full p-2 bg-red-600 cursor-pointer hover:bg-red-600 text-white font-bold"
              onClick={() => {
                router.push('/profile?tab=addresses')
              }}
            >
            ...
            <>
              <button className="w-full p-2 flex gap-2 justify-center bg-red-600 cursor-pointer hover:bg-red-600 text-white font-bold"
